...
bool Search_tree<Type>::insert( Type const &obj ) {
	if ( empty() ) {
		...
		Node *ptr = front_sentinel;
		
		while (obj > ptr->next_node->node_value && ptr->next_node!= back_sentinel){
			ptr= ptr->next_node;
		}
		
		Node *after_node = ptr->next_node;
		Node *new_node = root_node->find(obj);
		ptr->next_node = new_node;
		new_node->previous_node = ptr;
	...
	if ( obj < node_value ) {
		if ( left_tree == nullptr ) {
			...
			if ( left_tree->insert( obj, left_tree ) ) {
				...
				if(left_tree->height() - right_tree->height() >1){	
					...
					else if (left_tree->right_tree->height() > left_tree->left_tree->height()){
		...
		if ( right_tree == nullptr ) {
			...
			if ( right_tree->insert( obj, right_tree ) ) {
				...
				if (right_tree->height() - left_tree->height() >1){
