...
int Tester<Class_name>::run() {
	...
	while ( true ) {
		...
		// Remove any comments
		if ( command.substr( 0, 2 ) == "//" || (comment_mode && command != "*/")) {
			char comment[1024];
		...
		// Block comments
		if (command == "/*"){
			std::cout << command << " Block comment start" << std::endl;
			std::cout << "..." << std::endl;
			comment_mode = true;
			continue;
		}
		if (command == "*/" && comment_mode){
			std::cout << command << " Block comment end" << std::endl;
			comment_mode = false;
			continue;
		}

		if (std::cin.fail()){
			std::cout << "Bad test file (probably missing an argument in the previous line or something similar)" << std::endl;
			break;
		}
		if ( std::cin.eof() ) {
